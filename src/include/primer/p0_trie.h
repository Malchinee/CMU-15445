...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    if (key.size() > 1) {
      for (char ch : key.substr(0, key.size() - 1)) {
        auto next = pre_node->get()->GetChildNode(ch);
        if (next == nullptr) {
          next = pre_node->get()->InsertChildNode(ch, std::make_unique<TrieNode>(ch));
        }
        pre_node = next;
      }
    }
    char ch = key[key.size() - 1];
    auto term_node = pre_node->get()->GetChildNode(ch);
    bool ret;

    // 相应节点不存在：插入返回成功
    if (term_node == nullptr) {
      pre_node->get()->InsertChildNode(ch, std::make_unique<TrieNodeWithValue<T>>(ch, value));
      ret = true;
    } else if (!term_node->get()->IsEndNode()) {
      // 相应节点存在但不是end＿node
      auto new_node = new TrieNodeWithValue<T>(std::move(*(*term_node)), value);
      term_node->reset(new_node);
      ret = true;
    } else {
      ret = false;
    }
  ...
  bool Remove(const std::string &key) {
    ...
    while (c != key.end()) {
      auto curr = c++;
      if (pre_node->get()->HasChild(*curr)) {
        s.emplace(*curr, pre_node);
        pre_node = pre_node->get()->GetChildNode(*curr);
        continue;
      }
      // 存在key没有的现象
      latch_.WUnlock();
    ...
    while (!s.empty()) {
      ...
      s.pop();
      auto remove_key = std::get<0>(temp);
      auto del_node = std::get<1>(temp);
      auto flag = (*del_node)->GetChildNode(remove_key);
      // 如果被删除的节点没有孩子，可以直接删除
      if (flag != nullptr && (*flag)->HasChildren()) {
        continue;
      }
      (*del_node)->RemoveChildNode(remove_key);
    }
    latch_.WUnlock();
    return true;
  }
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    auto pre_node = &root_;
    for (char ch : key) {
      auto next_node = pre_node->get()->GetChildNode(ch);
    ...
    auto term_node = dynamic_cast<TrieNodeWithValue<T> *>(pre_node->get());
    T ret;

    if (term_node != nullptr) {
      *success = true;
      ret = term_node->GetValue();
    } else {
      *success = false;
      ret = {};
    }
